...
- **Affordance-Grounded Prompting**:
  ...
  - **Native Structured JSON Output**: Groq/Cerebras/OpenRouter/Nvidia receive `response_format: {type: 'json_object'}` and Gemini `responseMimeType: 'application/json'` for REASONING/PLAN/RESEARCH/SOCIAL_CHAT/EMOTION tasks; regex-based `parseLLMResponse` remains as universal fallback.
